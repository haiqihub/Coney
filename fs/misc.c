...
/**
 *****************************************************************************/
PUBLIC int split_dir_file(struct inode **pp_inode, const char *pathname, char *filename){
	struct inode *res = 0;
	char *f = filename;
		...
		if(*path == '/' && f > filename){
			*f = '\0';
			...
			if(strcmp(filename, "..") == 0){
				*pp_inode = (*pp_inode)->i_parent;
			}
			int inodenr = get_inode_nr(*pp_inode, filename);
			if(!inodenr){
				/* file non-exist */
				*pp_inode = 0;
				return INVALID_PATH;
			}
			res = get_inode((*pp_inode)->i_dev,inodenr);
			if(fs_msg.type != MLCD || fs_msg.FLAGS != O_RDWR){
				/* the cmd isn't cd */
				put_inode(*pp_inode);
			}
			if(res->i_mode != I_DIRECTORY){
				*pp_inode = 0;
				return INVALID_PATH;
			}
			*pp_inode = res;
...
/**
 *****************************************************************************/
PUBLIC int get_inode_nr(struct inode *dir_inode, char *filename)
{
	int dir_blk0_nr = dir_inode->i_start_sect;
	int nr_dir_blks = (dir_inode->i_size + SECTOR_SIZE - 1) / SECTOR_SIZE;
 ...
 ****************************************************************************/
/**
 * get all dir and file in this inode
 *****************************************************************************/

PUBLIC int get_all_dir(struct inode *dir_inode,char dir_name[20][12])
{
	int dir_blk0_nr = dir_inode->i_start_sect;
	int nr_dir_blks = (dir_inode->i_size + SECTOR_SIZE - 1) / SECTOR_SIZE;
	int nr_dir_entries = dir_inode->i_size / DIR_ENTRY_SIZE;
	...
	for (int i = 0; i < nr_dir_blks; i++) {
		...
		pde = (struct dir_entry *)fsbuf;
		for (int j = 0; j < SECTOR_SIZE / DIR_ENTRY_SIZE; j++,pde++) {
			c=*(dir_name+dir_file_count);
			memcpy(c,pde->name, MAX_FILENAME_LEN);
			dir_file_count++;
			//dir_file++;
			if (++m > nr_dir_entries)
				break;
