...
PUBLIC void task_fs()
{
	...
	while (1) {
		...
		case STAT:
			...
			break;
		/* Mutiple Level FileSystem */
		case MLOPEN:
		case MLCD:
			fs_msg.RETVAL = do_mlopen();
			break;
		case UNMLLINK:
			fs_msg.RETVAL = do_unmllink();
			break;
		case MLLS:
			fs_msg.RETVAL = 0;
			int cnt  = do_mlls(dir_name);
			phys_copy(va2la(src, *fs_msg.DIR_NAME),va2la(TASK_SYS, *dir_name),12*cnt);
			fs_msg.CNT=cnt;
			break;
		case LEVEL:
			fs_msg.RETVAL = dirlevel;
			break;
		default:
...
PRIVATE void init_fs()
{
	...
	if (sb->magic!= MAGIC_V2) {//sb->magic!= MAGIC_V2
		printl("{FS} mkfs\n");
		mkfs(); /* make FS */
	}
	printl("{FS} mkfs(Done)\n");
	...
	char *path = root_inode->pathname;
	*path++ = '/';
	*path = 0;
	root_inode->path_len = 2;
}
...
PRIVATE int fs_exit()
{
	...
	for (i = 0; i < NR_FILES; i++) {
		if (p->filp[i]) {
			struct inode *pin = p->filp[i]->fd_inode;
			if (--pin->i_used == 0){
				...
				if (imode == I_CHAR_SPECIAL) {
					...
					assert(MAJOR(dev) == 4);
					assert(dd_map[MAJOR(dev)].driver_nr != INVALID_DRIVER);
					send_recv(BOTH, TASK_TTY, &driver_msg);
				}
			}
			/* release the inode */
