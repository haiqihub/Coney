...
PRIVATE void    tty_do_close    (TTY* tty, MESSAGE* msg);
PRIVATE void    tty_do_open     (TTY* tty, MESSAGE* msg);
PRIVATE void    tty_getname     (MESSAGE *msg);
PRIVATE void    tty_do_cache    (TTY* tty, MESSAGE* msg);
PRIVATE void    tty_conf_dir    (MESSAGE *msg);
...
PUBLIC void task_tty()
{
	...
	while (1) {
		...
		case DEV_WRITE:
			...
			break;
		case DEV_CLOSE:
			tty_do_close(ptty, &msg);
			break;
		case DEV_GETNAME:
			tty_getname(&msg);
			break;
		case DEV_CACHE:
			tty_do_cache(ptty, &msg);
			break;
		case DEV_DIR:
			tty_conf_dir(&msg);
			break;
		case SELCON:
			select_console(msg.CNT);
...
PUBLIC void in_process(TTY* tty, u32 key)
{
	if (!(key & FLAG_EXT)) {
		...
		case PAGEDOWN:
			...
			break;	
		case RIGHT:
                        change_cursor(tty->console,0);
			if(insert_cnt)
				insert_cnt--;
                        break;
                case LEFT:                     
                        change_cursor(tty->console,1);
			insert_cnt++;
                        break; 
		case F1:
...
PRIVATE void put_key(TTY* tty, u32 key)
{
	if(tty->imode == ICONSOLE){
		...
		if(tty->itext_cnt < TTY_TEXT_BYTES){
			*(tty->itext + tty->itext_cnt) = key;
			tty->itext_cnt++;
		}
	}
}
...
PRIVATE void tty_dev_write(TTY* tty)
{
	while (tty->ibuf_cnt || tty->itext_cnt - tty->itext_pos) {
		char ch;
		if(tty->imode == ICONSOLE){
			ch = *(tty->ibuf_tail);
			tty->ibuf_tail++;
		...
		if (tty->tty_left_cnt) {
			if (ch >= ' ' && ch <= '~') { /* printable */
				if (flag == 1){				
					out_char2(tty->console, ch);
					int temp = insert_cnt;
					void * p = tty->tty_req_buf +
						   tty->tty_trans_cnt;
					void * r = tty->tty_req_buf + tty->tty_trans_cnt - 1;
					for(; temp > 0; temp--){
						phys_copy(p, r, 1);
						p = p - 1;
						r = r - 1;
					...
					r = r + 1;		
					phys_copy(r, (void *)va2la(TASK_TTY, &ch), 1);
					tty->tty_trans_cnt++;
					tty->tty_left_cnt--;
				}
				else{
			...
			if (tty->imode == ICONSOLE){
				if (ch == '\b' && tty->tty_trans_cnt) {
					if(flag == 1){
						out_char2(tty->console, ch);
						void * p = tty->tty_req_buf + tty->tty_trans_cnt - insert_cnt;
						void * r = p + 1;	
						int temp = insert_cnt - 1;
						for( ; temp > 0 ; temp--){
							phys_copy(p, r, 1);
							p = p + 1;
							r = r + 1;
			...
			else if (tty->imode == ITEXT){
				if (ch == '\b' || ch == '\n' || tty->tty_left_cnt == 0){
					out_char(tty->console, ch);
					phys_copy(tty->tty_req_buf + tty->tty_trans_cnt, (void *)va2la(TASK_TTY, &ch), 1);
					tty->tty_trans_cnt++;
					MESSAGE msg;
					msg.type = RESUME_PROC;
					msg.PROC_NR = tty->tty_procnr;
...
PRIVATE void tty_do_write(TTY* tty, MESSAGE* msg)
{
	...
	int j;
	if (tty->imode == ITEXT){
		int bytes = min(TTY_TEXT_BYTES, i);
		phys_copy((void*)tty->itext, (void*)p, bytes);
		tty->itext_cnt = tty->itext_pos = bytes;
	}
	while (i) {
...
PRIVATE void tty_do_close(TTY* tty, MESSAGE *msg)
{
	int pos = tty - TTY_FIRST;
	int src = msg->source;
	assert(tty_ctrl[pos]);
	if (pos != 0){
		clear_screen(tty->console->orig, tty->console->con_size);
		tty_ctrl[pos] = 0;
		select_console(0);
	}
...
PRIVATE void tty_getname(MESSAGE *msg){
	...
	for (i = 1; i < NR_CONSOLES; i++){
		if(tty_ctrl[i] == 0){
			sprintf(name, "/dev_tty%d", i);
			phys_copy(va2la(msg->source, msg->PATHNAME), va2la(TASK_TTY, name), 10);
			msg->FLAGS = 1;
			break;
		}
	}
...
PRIVATE void tty_do_cache(TTY* tty, MESSAGE* msg){
	char name[10];
	phys_copy(va2la(TASK_TTY, name), va2la(msg->source, msg->PATHNAME), msg->CNT);
	int pos = name[msg->CNT - 1] - '0';
	if(pos != 0){
		(TTY_FIRST + pos)->imode = ITEXT;
		(TTY_FIRST + pos)->console->imode = ITEXT;
	}
	msg->FLAGS = 1;
	send_recv(SEND, msg->source, msg);
}
...
PRIVATE void tty_conf_dir(MESSAGE *msg){
	...
	if(pos != 0){
		if(!tty_dir[pos]){
			tty_dir[pos] = (void *)root_inode;
		}	
		switch (msg->FLAGS){
		case GET:
																																			
			msg->NODE = (void *)tty_dir[pos];
			break;
		case SET:
			tty_dir[pos] = (struct inode *)msg->NODE;
			break;
		case NAME:
			phys_copy(va2la(msg->source, msg->NODE), (void *)tty_dir[pos]->pathname, tty_dir[pos]->path_len);
			break;
		}
