...
PUBLIC void init_screen(TTY* tty)
{
	...
	tty->console->imode = ICONSOLE;
	tty->console->index_top = 0;
...
PUBLIC void out_char(CONSOLE* con, char ch)
{
	...
	switch(ch) {
	case '\n':
		*(con->bef_enter_index + con->index_top) = con->cursor;
		con->cursor = con->orig + SCR_WIDTH * (cursor_y + 1);
		*(con->aft_enter_index + con->index_top) = con->cursor;
		con->index_top++;
		break;
	case '\b':
		if (con->cursor > con->orig) {
			if(con->imode == ITEXT && con->index_top > 0 && con->cursor == *(con->aft_enter_index + con->index_top - 1)){
				con->index_top--;
				con->cursor = *(con->bef_enter_index + con->index_top);
			}
			else {
				con->cursor--;
			}
...
PUBLIC void out_char2(CONSOLE* con, char ch){
	
	u8* pch = (u8*)(V_MEM_BASE + con->cursor * 2);
	u8* pch1 = (u8*)(V_MEM_BASE + con->cursor * 2);
	...
	if (con->cursor <
		    con->orig + con->con_size - 1) {
                        ...
                        pch1 = (u8*)(V_MEM_BASE + con->cursor * 2);
...
PUBLIC void next_console(){
	while(++current_console < NR_CONSOLES && !tty_ctrl[current_console]){}
	if(current_console >= NR_CONSOLES) current_console = 0;
...
{   u8* p_vmem = (u8*)(V_MEM_BASE + p_con->cursor * 2);
     ...
     int i=2;
     while (p_con->cursor > p_con->orig +  SCR_WIDTH * 
				((p_con->cursor - p_con->orig) /
				 SCR_WIDTH ))
                {      
			p_con->cursor--;
			*(p_vmem-2) = ' ';
...
PUBLIC void setColor(){
	color++;
	if ( color == 1 ) {_color = 0x04;}   //red
	else if ( color == 2 ) {_color = 0x02;}
	else {_color = 0x07; color = 0;}
}
